...
class IperfTest(TempItem):
    ...
    def exe(self):
        ...
        self.wget(_iperf_rpm)
        work_path = self.root_path
        iperf_tool = f'{work_path}/Luxshare-AnconaRT1/100/IperfLoopTool.py'
        log_path = f'{work_path}/Luxshare-AnconaRT1/100/'
        self.os_run.run(f"rpm -ivh --nodeps --force /run/LuxScript/tools/ancoan/rpm/iperf-2.1.6-2.el8.x86_64.rpm")
        with self.ssh_connect(uut=self.config["UUT"]):
            ...
            for info in infos:
                ...
                if check_info < 10 :
                    ...
                    return Fail(self)
            # self.execute_run(f"mv {run}reports/iperf/ {mount_path}iperf_log/{server_num}_iperf")
