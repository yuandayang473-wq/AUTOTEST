...
class SwFwUpdate_1(TempItem):
    ...
    def exe(self):
        ...
        self.is_head_power_reset = False
            ...
            for i in range(1, 5):
                ...
                for ver in versions:
                    ...
                    if update_switch_fw(ver, i) == 0:
                        self.is_head_power_reset = True
                        break
                self.sleep(3)

        if self.is_head_power_reset:
            self.platform.put_platform_data(self.config["BMC_HEADER"])
        else:
            # 已经最新switch， 不用执行后面dynamic=true 的tool
            self.platform.skip_dynamic_tool("switch updated")
