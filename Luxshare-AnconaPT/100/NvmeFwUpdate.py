...
class NvmeFwUpdate(TempItem):
    ...
    def __init__(self):
        ...
        self.config = [
            ...
            {"folder": "Luxshare-AnconaPT/100/Config", "file": "UUT.yaml", "name": "nvme", "key": "tools/ancoan/fw/nvmefw"},
        ...
        with self.ssh_connect(uut=self.config["UUT"]):
            ...
            for key_word, nvme_type in data.items():
                ...
                for nvme in nvme_list:
                    if nvme.split('-')[1] != nvme_type:
                        if key_word == "intel":
                            self.execute_run(f"{nvme['tool']} fw-download {nvme.split('-')[0]} -f {nvme['intel']['fw']}")
                            self.execute_run(f"{nvme['tool']} fw-activate {nvme.split('-')[0]} -s 0 -a 3")
                        elif key_word == "samsung":
                            self.execute_run(f"{nvme['tool']} fw-download {nvme.split('-')[0]} -f {nvme['samsung']['fw']}")
                            parser = self.execute_run(f"{nvme['tool']} fw-activate {nvme.split('-')[0]} -s 0 -a 3")
                            if not parser.check_field("Success"):
                                self.fail(f"{key_word} update fail")
