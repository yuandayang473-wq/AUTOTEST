...
class OamFruWrite(TempItem):
    ...
    def __init__(self):
        ...
        self.config = [
            ...
            {"folder": "Luxshare-AnconaPT/100/Config", "file": "UUT.yaml", "name": "relation", "key": "relation_info"},
        ]
    ...
    def exe(self):
        ...
        with self.ssh_connect(uut=self.config["UUT"]):
            ...
            if not re.search(r"Stopping IPMI Stack", parser.get_origin_data(), re.I):
                ...
                self.fail(ErrorCode.FFFFFFFF, "IPMIMain process stop fial")
            self.sleep(3)
            parser = self.execute_run(
            ...
            for fru in fru_info:
                ...
                if current_fruinfo[0] != fru["oam_sn"]:
                    ...
                    self.fail(ErrorCode.FFFFFFFF, "Device {device_dict[fru['slot']]} Sn number is not match")
                if current_fruinfo[1] != fru["oam_part_no"]:
                    self.logger.info("Device {device_dict[fru['slot']]} Sn Part number is not match")
                    self.fail(ErrorCode.FFFFFFFF, "Device {device_dict[fru['slot']]} Sn Part number is not match")
