    ...
    def exe(self):
        ...
        with self.ssh_connect(uut=self.config["UUT"]):
            ...
            for key_word, nvme_type in data.items():
                parser = self.execute_run(f"{nvme['tool']} list | grep -i {key_word} | " + '''awk '{print $1 "-" $NF}' | xargs ''')
                nvme_list = parser.get_origin_data().split()

                for nvme in nvme_list:
                    self.assertEqual(ErrorCode.FFFFFFFF,f"check device {nvme.split('-')[0]} fw version", nvme.split('-')[1].strip(), nvme_type.strip())
