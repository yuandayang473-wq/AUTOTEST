...
class HWQC(TempItem):
    ...
    def __init__(self):
        ...
        self.config = [
            ...
            {"folder": "Luxshare-AncoanRT/100/Config", "file": "UUT.yaml", "name": "rpm_info", "key": "tools.ancoan.rpm"},
            {"folder": "Luxshare-AncoanRT/100/Config", "file": "UUT.yaml", "name": "hwqc", "key": "tools/ancoan/HWQC"},
        ]
    ...
    def init_env(self):
        ...
        self.os_run.run(f"cd {path};wget -t 5 -T 60 -r -np -nH -R index.html {_hwqc}")
        # self.os_run.run(f"cd {path};wget -t 5 -T 60 -r -np -nH -R index.html {ali_driver}")
        self.os_run.run(f"cd {path};wget -t 5 -T 60 -r -np -nH -R index.html {alippu_driver}")
