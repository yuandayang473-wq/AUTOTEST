...
class TestLink:
    ...
    def _run_speed_case(self, speed_code: int, speed_name: str):
        for ep_bdf, dsp_bdf in self.ep_dsp_pairs:
            METHOD.speed_change(dsp_bdf, speed_code)
            cap_speed, cap_width, current_speed, current_width = METHOD.get_speed_width(ep_bdf)
            LOGGER.info("速率变化后ep_bdf:{} cap_speed:{} current_speed:{}".format(
                ep_bdf, cap_speed, current_speed
            ))
    ...
    @pytest.mark.author("袁大阳")
    def test_pcie_sys_link_004(self):
        with BASE.ssh_connect(uut=self.config.config["UUT"]):
            self._run_speed_case(1, "2.5GT/s")
            self._run_speed_case(2, "5GT/s")
            self._run_speed_case(3, "8GT/s")
            self._run_speed_case(4, "16GT/s")
            self._run_speed_case(5, "32GT/s")
