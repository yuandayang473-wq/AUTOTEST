...
class Rt2StationCrossing(TempItem):

    def __init__(self):
        super().__init__()
        self.name = "rt2  hwqc log upload "
    ...
    def exe(self):
        ...
        with self.ssh_connect(uut=self.config["UUT"]):
            ...
            time_struct = time.strptime(struct_time, from_format)
            timename = time.strftime(to_format_2, time_struct)
            station = "LXKS_K02-2FFATP-01_1_RT2"
            sn = self.parent.globals['SN']
            ...
            payload = {"cmd": "ADD", "empNo": "", "terminalName": station,
                   ...
                   "machineNo": "", "toolingNo": "", "cavityNo": "", "result": "PASS", "defectCode": "",
                    "uut_start": self.parent.globals['start_time'], "uut_stop": stoptime, 
                    "limits_version": "", "software_name": "", "software_version": ""}
            jbog_sn = _mes.get_mes_info(self.parent.globals["SN"])["Results"]["jbog_sn"]
            server_sn = _mes.get_mes_info(self.parent.globals["SN"])["Results"]["server_sn"]
            ...
            cmd = f'cd /mnt/HWQC_log/{server_sn} ; zip -r -q /LogFile/Luxshare-PPU_HWQC_Test/{server_sn}{timename}.zip hwqc.{server_sn}.log hwqc.{server_sn}.report' 
            self.execute_run(cmd)
            cmd = f'cd /mnt/HWQC_log/{server_sn} ; zip -r -q /LogFile/Luxshare-PPU_HWQC_Test/{jbog_sn}{timename}.zip hwqc.{jbog_sn}.log hwqc.{jbog_sn}.report' 
            self.execute_run(cmd)
...
if __name__ == '__main__':
    runner.single_runner(Rt2StationCrossing)
