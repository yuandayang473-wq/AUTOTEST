...
class HibBoardPartNumber(TempItem):
    ...
    def exe(self):
        with self.ssh_connect(uut=self.config["UUT"]):
            ...
            write_info =  _mes.get_mes_info(self.mes_info["info"]["sn"])["Results"]["hib_part_number"]
            ...
            self.assertEqual(ErrorCode.FFFFFFFF, f" Hib Chassis Board Part Number ", write_info, parser)
            # self.assertEqual(ErrorCode.FFFFFFFF, f"clear bmc sel log", int(1), len(count))
        


if __name__ == '__main__':
    runner.single_runner(HibBoardPartNumber)
