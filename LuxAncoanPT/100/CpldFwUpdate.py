...
class CpldFwUpdate(TempItem):
    ...
    def exe(self):
        ...
        with self.ssh_connect(uut=user):
            ...
            if current_versions['ubb1'] != FwVsersion.get("ubb1"):
                ...
                if not re.search(r'Flash\s*Complete', parser.get_origin_data(), re.I):
                    ...
                    self.fail(ErrorCode.FWTUP003, "Tail ubb1 flash bure fail")
            else:
                flag = "un update"
            ...
            if current_versions['ubb2'] != FwVsersion.get("ubb2"):
                ...
                if not re.search(r'Flash\s*Complete', parser.get_origin_data(), re.I):
                    ...
                    self.fail(ErrorCode.FWTUP003, "Tail ubb2 flash bure fail")
            else:
                flag = "un update"
            ...
            if current_versions['fanbp80'] != FwVsersion.get("fanbp80"):
                ...
                if not re.search(r'Flash\s*Complete', parser.get_origin_data(), re.I):
                    ...
                    self.fail(ErrorCode.FWTUP003, "Tail 80 fan board flash bure fail")
            else:
                flag = "un update"
            ...
            if m_config == "W scaleout":
                ...
                if current_versions['fanbp40'] != FwVsersion.get("fanbp40"):
                    ...
                    if not re.search(r'Flash\s*Complete', parser.get_origin_data(), re.I):
                        ...
                        self.fail(ErrorCode.FWTUP003, "Tail 40 fan board flash bure fail")
                else:
                    flag = "un update"
            ...
            if current_versions['nvmebp1'] != FwVsersion.get("nvmebp1"):
                ...
                if not re.search(r'Flash\s*Complete', parser.get_origin_data(), re.I):
                    ...
                    self.fail(ErrorCode.FWTUP003, "Tail nvmebp1 flash bure fail")
            else:
                flag = "un update"
            ...
            self.logger.info(
                f"Current ubb2 cpld version is: {current_versions['nvmebp2']}, target version is {FwVsersion.get('nvmebp2')}")
            if current_versions['nvmebp2'] != FwVsersion.get("nvmebp2"):
                ...
                if not re.search(r'Flash\s*Complete', parser.get_origin_data(), re.I):
                    ...
                    self.fail(ErrorCode.FWTUP003, "Tail nvmebp2 flash bure fail")
            else:
                flag = "un update"
        ...
        with self.ssh_connect(uut=self.config["UUT"]):
            # check HIB fw verion
            self.execute_run("rm -rf /root/BMCLog", i_exit_code=True)
            self.logger.info(
        ...
        with self.ssh_connect(uut=user):
            ...
            if current_versions['cpuboard_ver'] != FwVsersion.get("cpuboard_ver"):
                ...
                if not re.search(r'Flash\s*Complete', parser.get_origin_data(), re.I):
                    ...
                    self.fail(ErrorCode.FWTUP003, "Tail cpubord flash bure fail")
            else:
                flag = "un update"
            ...
            if current_versions['cmuboard_ver'] != FwVsersion.get("cmuboard_ver"):
                ...
                self.flash_flag = True
                parser = self.execute_run(
                    f"{path['header_cpld_script']} {header_bmc_ip} taobao 9ijn0okm {path['cmu_cpld_fw']}")
                if not re.search(r'Flash\s*Complete', parser.get_origin_data(), re.I):
                    self.logger.info("Tail cmu flash bure fail")
                    self.fail(ErrorCode.FWTUP003, "Tail cmu flash bure fail")
            else:
                flag = "un update"
    ...
    def tearDown(self):
        ...
        with self.ssh_connect(uut=user):
            ...
            for i in range(1, 11):
                ...
                self.sleep(10)
            if not flag1:
                self.fail(ErrorCode.FFFFFFFF, "Server power off statu: fail")
            # 机尾下点后delay 30s
            self.sleep(30)
            ...
            for i in range(1, 11):
                ...
                self.sleep(10)
            if not flag2:
                self.fail(ErrorCode.FFFFFFFF, "Jbog power off statu: fail")
            ...
            for i in range(1, 11):
                ...
                self.sleep(10)
            if not flag3:
                self.fail(ErrorCode.FFFFFFFF, "Jbog power on statu: fail")
            ...
            for i in range(1, 11):
                ...
                self.sleep(10)
            if not flag4:
                self.fail(ErrorCode.FFFFFFFF, "Server power on statu: fail")
