...
class HibChassisExtra(TempItem):
    ...
    def exe(self):
        with self.ssh_connect(uut=self.config["UUT"]):
            ...
            _mes = MesSocket()
            write_info =  _mes.get_mes_info(self.parent.globals["SN"])["Results"]["jbog_customer_part_number"]
            ...
            self.assertEqual(TypeCode.FFFFFFFF, f" Hib Chassis board Extra ", write_info, parser)
            # self.assertEqual(TypeCode.FFFFFFFF, f"clear bmc sel log", int(1), len(count))
        


if __name__ == '__main__':
    runner.single_runner(HibChassisExtra)
