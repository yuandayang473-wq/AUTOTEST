...
class OamPowerFw_Bat(TempItem):
    ...
    def exe(self):
        ...
        with self.ssh_connect(uut=self.config["UUT"]):
            ...
            if len(pmbus_list) == 0:
                self.fail(ErrorCode.FFFFFFFF, "Get pmbus info fail")
            ...
            for pmbus in pmbus_list:
                if pmbus['ver'] != "0x3":
                    ...
                    if not re.search(r'pmbus\s*operation\s*successful', parser.get_origin_data(), re.I):
                        self.fail(ErrorCode.FFFFFFFF, f"Device:{pmbus['dev']} power pmbus operation fail, for ver")
                    self.sleep(1)
                if pmbus['ocl1'] != "0x2186":
                    parser = self.execute_run(f'''ppudbg --mpmbop write 0 0x20 0 0xb9 0x2186 "force" --device {pmbus['dev']}''')
                    if not re.search(r'pmbus\s*operation\s*successful', parser.get_origin_data(), re.I):
                        self.fail(ErrorCode.FFFFFFFF, f"Device:{pmbus['dev']} power pmbus operation fail, for ocl1")
                    self.sleep(1)
                if pmbus['CRC'] != "0xf522":
                    parser = self.execute_run(f'''ppudbg --mpmbop write 0 0x20 1 0xf0 0xf522 "force" --device {pmbus['dev']}''')
                    if not re.search(r'pmbus\s*operation\s*successful', parser.get_origin_data(), re.I):
                        self.fail(ErrorCode.FFFFFFFF, f"Device:{pmbus['dev']} power pmbus operation fail, for CRC")
                    self.sleep(1)
                ...
                if not re.search(r'operation\s*successful', parser.get_origin_data(), re.I):
                    self.fail(ErrorCode.FFFFFFFF, f"Device:{pmbus['dev']} program dul fail")
                self.sleep(1)
