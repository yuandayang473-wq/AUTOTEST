...
class HibChassisPartNumber(TempItem):
    ...
    def __init__(self):
        ...
        self.config = [
            ...
            {"file": "BmcDevice.yaml", "name": "SBMC", "key":"BMC_02"},{"file": "BmcDevice.yaml", "name": "JBMC", "key":"BMC_02"},
        ]
    def exe(self):
        with self.ssh_connect(uut=self.config["UUT"]):
            ...
            self.assertEqual(ErrorCode.FFFFFFFF, f"Hib Chassis Part Number ", write_info, parser)
            # self.assertEqual(ErrorCode.FFFFFFFF, f"clear bmc sel log", int(1), len(count))
        


if __name__ == '__main__':
    runner.single_runner(HibChassisPartNumber)
